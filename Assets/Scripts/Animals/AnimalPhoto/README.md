...
2. Select that asset and open **Tools > Animal Game > Animal Photo Library**. Add state groups and image entries (Texture2D assets), then select **Edit** beside an image.
3. Drag over the image to define the required subject rectangle, or enter its normalized Rect. The origin is the bottom left. Green is the required subject area; orange is an example random crop. Set saturation (0 = grayscale, 1 = original, 2 = enhanced). **Preview another random crop** uses the same processing as runtime. Changes support Undo/Redo; **Save library** writes the asset.
...
Texture Read/Write is not required. Use individual source textures; do not supply a sprite atlas containing multiple photographs. Subject rectangles default to the whole image, which intentionally permits no random crop until you narrow the rectangle. Empty/missing matching libraries log a warning and skip the result. Legacy subject-local photos remain serialized for manual migration but are no longer selected by PhotoResultUI.

## Capture and rendering

PhotoModeController.PhotoCaptured invokes PhotoResultUI.HandlePhotoCaptured. Main-subject detection is unchanged. The library samples uniformly from valid images matching the captured species and state, then independently samples crop edges outside the required subject rectangle. Every crop stays within the image and contains that rectangle.
